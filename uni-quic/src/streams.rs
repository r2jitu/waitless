...
/// Small map keyed by `u64` stream id, backed by a Vec for
/// efficient point lookups when the entry count is small (≤
/// dozens). Replaces the previous `BTreeMap<u64, T>` usage in
/// `Connection` — every `insert` into a BTreeMap allocates a
/// fresh node, costing two heap allocs per H3 request stream
/// (one each for `recv_streams` + `send_streams`); a Vec-
/// backed map with pre-allocated capacity amortises that to
/// zero on the steady-state path.
///
/// Linear scan get/remove costs O(N) over the live entry
/// count. For QUIC connections the live set is normally 1-4
/// streams per conn (current request's bidi pair plus a
/// handful of peer-initiated uni streams for QPACK / control);
/// linear scan over 4 cache-line-friendly entries beats the
/// BTreeMap's pointer chasing in absolute time AND avoids the
/// alloc churn.
pub struct SmallStreamMap<V> {
    entries: Vec<(u64, V)>,
}

impl<V> SmallStreamMap<V> {
    pub fn with_capacity(cap: usize) -> Self {
        Self { entries: Vec::with_capacity(cap) }
    }

    pub fn contains_key(&self, sid: u64) -> bool {
        self.entries.iter().any(|(k, _)| *k == sid)
    }

    pub fn get(&self, sid: u64) -> Option<&V> {
        self.entries.iter().find(|(k, _)| *k == sid).map(|(_, v)| v)
    }

    pub fn get_mut(&mut self, sid: u64) -> Option<&mut V> {
        self.entries.iter_mut().find(|(k, _)| *k == sid).map(|(_, v)| v)
    }

    pub fn insert(&mut self, sid: u64, value: V) {
        // Caller is expected to have checked `contains_key` —
        // an insert with a duplicate key just appends a second
        // entry, which `get` then masks (returns the first hit).
        // For our single-writer per-stream-id pattern this is
        // fine; future tighten if the assumption breaks.
        self.entries.push((sid, value));
    }

    pub fn remove(&mut self, sid: u64) -> Option<V> {
        let idx = self.entries.iter().position(|(k, _)| *k == sid)?;
