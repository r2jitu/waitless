...
doesn't have to reconstruct them from commit archaeology:
  ...
  * **Phase 5 Step 6 shipped:** the `EthernetDriver` trait grew
    to 19 methods (probe + send + poll + 16 extended dispatcher
    extras), `drivers::net::*` trait-dispatches through an
    `AtomicPtr<EthernetDriverReg>` active slot, and `//drivers:
    drivers` dropped its static deps on the NIC crates. Apps
    declare which NIC crate(s) they want in their own BUILD
    `select()` deps + `extern crate uni_driver_*` in `main.rs`
    (the `#[used]` link-section entry is otherwise DCEd). Verified
    by symbol inspection: `hello.elf` now has 1 `.uni_drivers_
    ethernet` entry (virtio only), `webserver.elf` has 2.
...
| 5 | Ethernet driver carveouts | 🟢 | Step 6 bench (HVF 1c, facbfc0 vs 64f1cc0): health_max +0.9%, compute_max +3.6%, udp_peak +1.6%, health_tls_max +3.2% — all within HVF noise floor. Step 5 bench (3-run mean): health_max −1.1%, udp_peak neutral, health_tls_max −0.3%. Step 4 split (single run): health_max −0.18%, udp_peak +1.48%, health_tls_max +0.22%, tls_handshake_max +0.84% — within ±2% gate. | none | **Step 1**: trait + macro + walker + linker sections. **Step 1.5**: `//uni-net:uni_net` crate carveout. **Step 1.6**: `//uni-net:driver` leaf crate (cycle break). **Step 2**: `VirtioNetDriver impl EthernetDriver` + registration. **Step 3**: `gvnic` → `gve` rename + `GveDriver` impl + registration. **Step 5**: `Net::enable` walks `linked_ethernet_drivers()` before bring-up; returns `NetError::NoDriver` if the binary has no registered driver, `NetError::NoNic` if drivers are linked but no `probe()` succeeds. **Step 4**: `virtio_net.rs` → `//uni-driver-virtio-net`, `gve.rs` → `//uni-driver-gve`, both depending on a new `//drivers:infra` sub-target (MMIO + PCI + VirtIO transport + virtio-console). The outer `//drivers:drivers` keeps `drivers::net::*` dispatch + `pub use drivers_infra::*` re-export so 43 existing caller sites don't touch. **Step 6**: `EthernetDriver` trait grew from 4 to 19 methods (probe + send + poll + 16 extended: get_mac, num_queue_pairs, activate_multi_queue, enable_irq, poll_qp, flush_tx_staging, flush_tx_kick_if_dirty, irq_idle_supported, arm_rx_interrupts, has_pending_rx/tx, rearm_rx_napi, enable_deferred_tx_kick, poke_interrupt_status, rx_counts, rx_used_cursors) with sensible no-op defaults. `drivers::net::*` trait-dispatches through an `AtomicPtr<EthernetDriverReg>` active-driver slot installed by the first successful `probe()`. `//drivers:drivers` dropped its static deps on `//uni-driver-virtio-net` / `//uni-driver-gve`. Apps pick NIC crate(s) via BUILD `select()` + `#[cfg(target_os="none")] extern crate uni_driver_*;` in `main.rs` (the link-section entry is otherwise DCEd). Verified end-to-end: `hello.elf` now has 1 `.uni_drivers_ethernet` entry (virtio only); `webserver.elf` has 2. The Step-4 "theater" note — apps that don't want gve couldn't drop it — no longer applies. |
