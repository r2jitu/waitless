...
| 5 | Ethernet driver carveouts | 🟡 | none (prereqs only; no hot path touched) | none | **Step 1 landed**: `EthernetDriver` trait + `NicHandle` + `EthernetDriverReg` + `register_ethernet_driver!` macro + `linked_ethernet_drivers()` walker, all in `uni::net`. `.uni_drivers_ethernet` section added to `unikernel.ld`, `unikernel_arm64.ld`, `unikernel_limine.ld`. `llvm-nm` confirms `__start_uni_drivers_ethernet` / `__stop_uni_drivers_ethernet` symbols are emitted; walk returns an empty slice today (no driver crate registers yet). Native builds stub the walker to `&[]`. **Remaining steps**: (2) virtio-net implements `EthernetDriver` (keep in `drivers/`), (3) gvnic→gve rename + trait impl, (4) carve into `//uni-driver-virtio-net` / `//uni-driver-gve` crates, (5) `Net::enable` walks the section instead of the `drivers::net::use_gvnic()` branch. Each step is an independent PR atop the trait shape. |
