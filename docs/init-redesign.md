...
| 5 | Ethernet driver carveouts | 🟡 | Step 2 bench (3-run mean): health_max −0.4%, udp_peak +0.8%, health_tls_max −0.2% — within ±2% gate. | none | **Step 1 landed**: `EthernetDriver` trait + `NicHandle` + `EthernetDriverReg` + `register_ethernet_driver!` macro + `linked_ethernet_drivers()` walker. `.uni_drivers_ethernet` section added to `unikernel.ld`, `unikernel_arm64.ld`, `unikernel_limine.ld`. **Step 1.5 landed: `//uni-net:uni_net` crate carveout.** **Step 1.6 landed: `//uni-net:driver` leaf crate** (`uni_net_driver`) — pulled the driver contract (trait, macro, walker, NicHandle, error types) out of `uni_net` into a no-dep leaf so `drivers` can depend on the trait without the `drivers → uni_net → net → drivers` cycle. `uni_net` re-exports everything from the leaf (`uni_net::EthernetDriver`, `uni_net::NetError`, etc., unchanged). **Step 2 landed**: `drivers/virtio_net.rs` now `impl EthernetDriver for VirtioNetDriver` and registers via `uni_net_driver::register_ethernet_driver!(VIRTIO_NET_DRIVER)`. Added `virtio_net::probe_ok()` flag mirroring gvnic's. `llvm-nm` confirms `__stop - __start = 16 B` (one `EthernetDriverReg` registered) on both hvf and iso ELFs. The legacy `drivers::net::init()` path still runs at boot; the trait impl is a parallel adapter that Step 5 will flip `Net::enable` onto. **Remaining steps**: (3) gvnic→gve rename + trait impl, (4) carve into `//uni-driver-virtio-net` / `//uni-driver-gve` crates, (5) `Net::enable` walks the section instead of `drivers::net::use_gvnic()`. |
