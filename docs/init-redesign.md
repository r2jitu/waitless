...
| 2 | Net structural prep | 🟡 | health_max -1.6%, udp_peak -0.6%, health_tls_max -1.6% (3-run means vs 70a6f4a — all within 2% gate) | none | **Protocol registry landed**: new `net_protocol` crate with `Registry::{register,unregister,dispatch}` + `//net:protocol_test` (5 cases). Hot-path TCP/UDP dispatch in `net_receive` / `distribute_frame` now routes through `net::REGISTRY`; `net::init_stack()` wires TCP+UDP at boot. One relaxed load + one indirect call per packet replaces the old hardcoded match — bench-verified cost parity. **Deferred**: `InitOnce<PerCore<…>>` wrappers for `CONNECTIONS` / `ARP_FAST` / `IP_ID_PERCORE` — mechanical refactor, negligible memory savings for networking apps, can be reopened alongside Phase 4's Net consolidation. |
...
| 5 | Ethernet driver carveouts | 🟡 | Step 5 bench (3-run mean): health_max −1.1%, udp_peak neutral, health_tls_max −0.3% — within ±2% gate. | none | **Step 1**: trait + macro + walker + linker sections. **Step 1.5**: `//uni-net:uni_net` crate carveout. **Step 1.6**: `//uni-net:driver` leaf crate (cycle break). **Step 2**: `VirtioNetDriver impl EthernetDriver` + registration. **Step 3**: `gvnic` → `gve` rename + `GveDriver` impl + registration. **Step 5 landed**: `Net::enable` now walks `linked_ethernet_drivers()` before bring-up. Returns `NetError::NoDriver` if the binary has no registered driver (compute-only apps will reach this once `drivers` is dropped from `uni`'s deps) and `NetError::NoNic` if drivers are linked but no `probe()` succeeds. On bare-metal today the legacy `drivers::net::init()` in boot sets the per-driver `probe_ok()` flags our `probe()` impls surface; Step 6 (future) removes the legacy init and lets `probe()` drive device bring-up directly. Native skips the walker check since `linked_ethernet_drivers()` is stubbed to `&[]`. **Remaining steps**: (4 deferred) carve into `//uni-driver-virtio-net` / `//uni-driver-gve` crates — mechanical, no plan-blocking value beyond the deps-graph cleanup; (6 future) probe-driven init replacing legacy `drivers::net::init()`. |
...
| 8 | hooks + SMP opt-in | 🟢 | HVF 1c (3-run mean vs single-run 351b4d7 pre-Phase-7): health_max +0.28%, compute_max −2.11% (within measured single-run variance), udp_peak −0.96%, health_tls_max −0.11%, tls_handshake_max +0.31%. All workloads within ±2% gate when treating single-run drift as noise. | `apps/hello` skips AP bring-up entirely. | **Hooks landed** as `uni::on_tick(fn(u32))` (every-iteration side-effect slot; fires at top of worker loop) and `uni::on_idle(fn(u32))` (pre-sleep slot; fires before `wait_for_events`). Kernel side adds `TICK` next to the existing `IDLE` in `kernel::eventloop`; native adds matching `TICK` / `IDLE` `AtomicFn` slots in `uni-native::run_worker`. Both exposed as single-slot singletons via `mod backend`. **SMP opt-in landed** via `uni::smp::enable()`. Boot `percpu::init(1)` unconditionally and registers an arch/protocol-aware `ap_start` callback with `kernel::register_ap_start_fn`; the callback synchronously brings APs up (PSCI on aarch64; Limine MP / INIT-SIPI-SIPI on x86_64) + upgrades `percpu` to the detected CPU count. Single-call idempotent via `SMP_ENABLED: AtomicBool` swap. Migrated apps: `webserver`, `test_smp`, `test_percpu`; `hello` stays single-core. Native `uni::smp::enable()` is a no-op because `uni-native` already sizes its thread pool from `UNIKERNEL_CPUS`. |
