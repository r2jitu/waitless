...
| 5 | Ethernet driver carveouts | 🟡 | Step 3 bench (3-run mean): health_max −1.0%, udp_peak −0.6%, health_tls_max −0.2% — within ±2% gate. | none | **Step 1**: `EthernetDriver` trait + registration macro + walker + linker-section additions. **Step 1.5**: `//uni-net:uni_net` crate carveout. **Step 1.6**: `//uni-net:driver` leaf (`uni_net_driver`) — pulled the driver contract out of `uni_net` so `drivers` can depend on it without the `drivers → uni_net → net → drivers` cycle. **Step 2**: `drivers/virtio_net.rs::VirtioNetDriver impl EthernetDriver` + `register_ethernet_driver!`. **Step 3 landed: `drivers/gvnic.rs` renamed to `drivers/gve.rs`** (driver module and `GveDriver` struct match Linux / Google's upstream driver name; GCE's product is still branded "gVNIC" in prose). `use_gvnic()` in the dispatch shim → `use_gve()`; NicInfo label `"gvnic"` → `"gve"`. `GveDriver` also registers via `register_ethernet_driver!(GVE_DRIVER)`. `llvm-nm` confirms `__stop - __start = 32 B` (two `EthernetDriverReg`s registered: `drivers::gve::ETHERNET_DRIVER_REG` + `drivers::virtio_net::ETHERNET_DRIVER_REG`) on both hvf and iso ELFs. The legacy `drivers::net::init()` path still runs at boot; the trait impls are parallel adapters that Step 5 will flip `Net::enable` onto. **Remaining steps**: (4) carve into `//uni-driver-virtio-net` / `//uni-driver-gve` crates, (5) `Net::enable` walks the section instead of `drivers::net::use_gve()`. |
