...
| 5 | Ethernet driver carveouts | 🟡 | health_max −1.7% (median-based), udp_peak neutral, health_tls_max neutral — within ±2% gate but near the noise floor | none | **Step 1 landed**: `EthernetDriver` trait + `NicHandle` + `EthernetDriverReg` + `register_ethernet_driver!` macro + `linked_ethernet_drivers()` walker. `.uni_drivers_ethernet` section added to `unikernel.ld`, `unikernel_arm64.ld`, `unikernel_limine.ld`. `llvm-nm` confirms `__start_*` / `__stop_*` symbols are emitted; walk returns empty today (no driver registered yet). Native stubs the walker to `&[]`. **Step 1.5 landed: `//uni-net:uni_net` crate carveout.** Moved `uni::net` module (Net/enable/NetBringUp/Ipv4Addr) + `uni::error` (NetError/DhcpError/NicError) + the Phase-5 driver-registration surface into the new crate. `uni` re-exports via `pub extern crate uni_net as net;` so `uni::net::Net` and `uni::NetError` paths keep working. Unblocks the `drivers → uni_net` dep needed for Steps 2–4 (driver-crate impl of `EthernetDriver`) without creating a `drivers → uni` cycle. **Remaining steps**: (2) virtio-net implements `EthernetDriver` (keep in `drivers/` initially), (3) gvnic→gve rename + trait impl, (4) carve into `//uni-driver-virtio-net` / `//uni-driver-gve` crates with the registration macro at each crate root, (5) `Net::enable` walks the section instead of `drivers::net::use_gvnic()`. |
