...
|---|---|---|---|
| 7 | Cancel-safety | `Drop`-clears-waker on TcpRecv/RecvChunk/TcpSendChain; `WaitEvent` Drop-deregistration | ✅ COMPLETE 2026-06-10: both Drop halves landed, then the (c) helper — `executor::waker_slot::{WakerSlot, Parked}`: the spinlocked slot + `present` gate extracted from the two hand-rolled copies (`AsyncEvent`, UDP `WorkerInbox`), with a `Parked` RAII guard whose field-drop deregisters `will_wake`-precisely. Writing it surfaced and closed a 5th instance the audit missed: neither UDP recv future had a Drop — a `select!`-canceled parked recv stranded a stale waker in the inbox (pinning its task until the next datagram). Both now hold the guard as a field — cancel-safe by construction, no manual Drop impls |
| 1 | Capabilities | `Time` facade; QUIC off `tls::ticket` time | session-scope ✅ 2026-06-10: (a) `quic::time` seam — all ~17 µs-reads behind one mockable facade; (c) Entropy NAMED as a capability (design-history Design Principles): the seam already had the right shape — proto crates take seeds by value (`TlsServer::new_box`, `Connection::new_server`), ambient `waitless::rng`/`kernel_core::rng` reserved for the composition shell; (b) the worker-owned-state review rule recorded alongside it. The full end-state (per-instance worker-owned stacks, kernel shrinks to boot+mm+executor+drivers) remains the long arc |
| 2 | Simulation | parser fuzz targets; mock-clock loss suite; seeded loss-schedule property suite | session-scope ✅ 2026-06-10: fuzz-smoke on all 3 attacker-facing parsers; virtual-time loss/PTO suite (caught 2 real bugs); **seeded loss-schedule property suite** — 200 reproducible runs (100 seeds × 10%/35% loss, per-transmission re-droppable drops) driving the REAL machine (`record_sent_packet` → wire-round-trip `process_ack`/`detect_loss` → `send_pto_probe` → retx bookkeeping → cwnd gate) under the mock clock, asserting eventual completion, a derived 4×N no-storm bound (measured max 1.77×), probe bounds, cwnd floor, and bit-exact determinism. The netem class of validation now runs in ~30ms in-repo. Remaining long arc: a true two-endpoint sim (needs a QUIC client role) |
| 3 | Completion reactor | `trait Nic` (completion-shaped); DQO TX waker | first increment ✅ 2026-06-10: `NicOps` (struct of ~25 fn pointers) → `trait Nic` + `&'static dyn Nic` behind a thin `AtomicPtr<DynSlot>`; gve/virtio/null-object as ZST impls delegating to the existing fns; `Option<fn>` capabilities → default methods; link-time registry keeps working. The one open question — the +1 vtable load on the busy-poll dispatch — settled by GCE A/B (c3/DQO, fresh deploys per arm): /health-TLS @1024 conns 339.2K (fn-ptr, median of 3) vs 345.7K (dyn, median of 3); /static-1m 2500 vs 2514 req/s — NEUTRAL. Driver contract is now a trait: the completion-shaped methods (submission/completion rings, (b) idle-gated MSI-X wake, (c) DQO TX waker — **re-profiled on GCE 2026-06-10 and the motivating problem is GONE**: a saturating /static-1m run filled+completed 32.2 M TX descriptors with `tx_ring_full_drops=0`, `tx_miss_compl=0`, `gqi_tx_stall_drops=0` — T1-T3 ACK-clocked backpressure already removed the silent-drop cliff (c) was named for, so (c) is withdrawn, not built). (b) idle-gated MSI-X wake is the remaining latency/power increment, not a throughput one |
| 4 | Transport engine | TCP RACK built as the shared core | session-scope ✅ 2026-06-10 in three steps: shared `RecoveryEpisode` (NewReno + CUBIC); **`net_cc::loss::LossParams`** — the transport-agnostic detector (packet + time thresholds, caller-tick units, zero-alloc, exact deadline semantics) extracted by porting QUIC's RFC 9002 detector, QUIC re-homed with every pinned virtual-time test + the 200-seed property suite passing byte-identical; **TCP RACK landed as the second consumer** (RFC 8985 detection half, tcp-backlog L4): time-based marking on the SACK scoreboard (id = una-offset, packet threshold disabled — 6675 owns count-based), same on_loss/emit path as 6675, reordering timer in on_tcp_tick, `rack_marked` diag; 4 new scenarios (87 total), existing 83 byte-unmodified. Remaining: adaptive reo_wnd (S2); BBR lands once on this core when wanted. **GCE netem-validated 2026-06-10**: real c3→kvm 1% loss at 25/50 ms RTT, `rack_marked` Δ225 over the trial set, `stalls(<0.2 MB/s)=0/15` on both profiles — RACK fires on hardware and the tail-loss bimodality is gone |
| 5 | Flow steering | PMTUD ICMP cross-core routing via generalized inbox | first increment ✅ 2026-06-10: per-core `ctrl_inbox` + shared `CtrlMsg` pool in kernel_core (the RX lane mechanism, control twin), policy in `net_stack::ctrl` — a wrong-core PMTUD report (`NoConn` from the now-three-way `note_path_mtu`) broadcasts to all cores; the owner re-runs every RFC 5927 gate and applies, closing the `pmtu_dropped` wrong-core hole. The lane is message-generic by design: the next cross-core fact adds a `CtrlMsg` variant, not a bespoke channel. `pmtu_routed`/`ctrl_dropped` counters in the net /obs block. GCE 2026-06-10 confirmed the precondition (`num_queue_pairs=8` → RSS active → wrong-core delivery is physically reachable) and the counters read live; a real frag-needed wasn't triggered (the bench path has no MTU constriction), so the routing decision stays pinned by its deterministic unit test. An ownership *map* (vs rare-hint broadcast) = the continuation if a high-volume consumer arrives |
| 6 | Memory arenas | box QUIC `DirKeys`; h3 `ConnArena` pattern-setter | session-scope ✅ 2026-06-10 in three steps: per-packet DirKeys clone eliminated + all 9 key slots boxed (**`Connection` 17,944 → 4,008 B**, −13.9 KB/h3 conn); ALL 8 remaining cold clone sites (Initial/Handshake encoders, 3 close arms, 3 probe arms) converted to take-and-restore — key material now allocates at derivation only; **`ConnArena` landed** — the four per-conn Rc allocations (Connection, progress AsyncEvent, peer ip/port Cells) co-allocated into one 4,080-B block (4→1 allocs/conn, size pinned by a regression test window). The long arc: typed arena regions for the dynamic state (stream map, retx, reassembly) — needs allocator plumbing |
