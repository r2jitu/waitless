...
> **Status: shipped.** The QUIC **performance record**, in two parts:
> Part 1 — the bulk-throughput campaign (the pacing-cap fix, ~8–10× h3
> downloads; HW UDP-GSO on DQO; the upload-reassembly cliff fix; the
> cross-env before/after matrix). Part 2 — the h3 /health
> **small-response** cost, root-caused (~2.1× CPU/req of h1, diffuse
> orchestration; merged in from the retired `h3-health-cycle-profile.md`
> 2026-06-10). Per-mechanism plan state: the rx/tx trackers +
> [`gvnic.md`](gvnic.md) (which owns the GSO-is-DQO-only device fact).
>
...
## Part 2 — h3 /health small-response cost, root-caused
