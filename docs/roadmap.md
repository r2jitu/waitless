...
| Perf/scale | ~~High-conn-count TLS needs a clean re-measure~~ ✅ closed 2026-06-11: full two-loadgen sweep to **80 K live TLS conns** (server-verified, 530 K rps). The "overload ~16 K" was real and server-side — two runtime walls, both fixed: (1) timer-wheel 8-per-slot cap fired 30 s idle timeouts *immediately* under same-µs re-arm bursts (mass-closed live conns); (2) task arena 4096/worker capped service at 32 K conns (accepted conns never got a handler). Curve + method in benchmark-results. Remaining at ≥40 K: p99 tails (1.5–4 s) trail tokio's; conn-establishment storms need a >10 s client timeout | S2 | benchmark-results |
| Boot | **GCE boot pays a ~10 s DHCP wait** — the reply to the first DISCOVER is missed (likely an RX path/filter not ready or unicast-OFFER handling) and the retry cadence is 2 s × 5; local HVF boots to serving in ~3 ms. Diagnose the missed first OFFER, add exponential-but-fast early retries | S3 | benchmark-results "Boot time" |
| Robustness | **Extreme-conn OOM is fatal, not graceful.** 240 K live TLS conns are stable; ~280 K (≈ heap exhaustion at ~55 KB/conn) panics → guest shutdown, reproduced 2×. The refuse-at-90 %-heap admission guard misses at least one allocation path. Needs: serial-confirmed repro of the panic site, then make it refuse/shed. Lever for the ceiling itself = per-conn memory (rx_ring tiering, arena #6) | S1 | benchmark-results "connection ceiling" |
