...
- **Cooperative-drain shutdown** — ✅ *done (2026-06-09):* the event loop keeps servicing in-flight requests (and flushing responses) for a bounded `DRAIN_GRACE_MS` (750 ms) after shutdown is signalled, then powers off + the per-conn RST sweep. A hard cycle-counter deadline (never hangs); composes with — rather than replaces — the prompt-RST design (drain first, RST stragglers), so the `test_open_conn_resets_promptly_on_shutdown` regression still passes. *Remaining: a QUIC clean CONNECTION_CLOSE on drain.*
