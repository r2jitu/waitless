...
| Perf/scale | **High-conn-count TLS needs a clean re-measure.** A 2026-05 *server-bound* run (single loadgen) was dead-flat to 50 K conns; a 2026-06 *two-loadgen* run hit overload ~16 K (asymmetric splits, ms-tails — likely loadgen-edge, not server). The current faster-crypto TLS path hasn't been cleanly swept to 50 K — ceiling is **unverified**, not known. If a real ceiling shows, the lever is per-conn memory (arena, #6). | S2 | benchmark-results / architecture-audit |
