...
//
// Initialises every subsystem in dependency order, then calls uni_boot()
// which spawns the app's `#[uni::boot]` body as a task on core 0's arena.
...
// the emitting side (the `#[uni::boot]` macro output) gives us a
...
unsafe fn kernel_boot(info: &BootInfo) {
    ...
    // Must happen before `uni_boot` so the app can `spawn` / `sleep_us`.
    uni_kernel::runtime::init();

    klog!("\n[BOOT] All subsystems ready. Starting application.\n\n");
    // `uni_boot()` (generated by `#[uni::boot]`) spawns the app's
    // boot body on core 0's arena and returns immediately. The
