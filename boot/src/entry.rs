...
unsafe fn kernel_boot(info: &BootInfo) {
    ...
    klog!("mem: {} / {} MB heap (talc, single allocator)\n",
          mm::get_free_memory() / (1024 * 1024),
          mm::get_total_memory() / (1024 * 1024));
    #[cfg(target_arch = "x86_64")]
    klog!("irq: x86_64 GDT/IDT (256 vectors), xAPIC online\n");
    #[cfg(target_arch = "aarch64")]
    klog!("irq: aarch64 VBAR_EL1 set, GICv{} configured\n",
          uni_kernel::aarch64::fdt::info().gic_version);
    klog!("smp: {} workers allocated (PerCore TLS via {})\n", cpu_count,
          if cfg!(target_arch = "x86_64") { "GS_BASE" } else { "TPIDR_EL1" });
    #[cfg(target_arch = "x86_64")]
    klog!("console: 16550 UART @ COM1 (port-IO, byte-at-a-time)\n");
    ...
    klog!("runtime: {} workers × 64 task slots, timer wheels armed\n",
