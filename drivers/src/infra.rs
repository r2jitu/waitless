// drivers/infra.rs — Shared infrastructure crate (`drivers_infra`).
//
...
// target. The outer `//drivers:drivers` crate layers the NIC
// dispatcher over both, so apps keep seeing the single `uni_drivers::*`
// surface.
//
