...
void reset(uint64_t base) {
    arch::virtio_write8(base + REG_DEVICE_STATUS, 0);
}
...
bool Virtqueue::init(uint16_t queue_size_param, uint64_t base,
    ...
    uint64_t avail_size = 6 + 2 * (uint64_t)queue_size_;  // flags + idx + ring[] + used_event
    uint64_t used_size  = 6 + 8 * (uint64_t)queue_size_;  // flags + idx + ring[] + avail_event
...
int Virtqueue::add_buf(void** buffers, uint32_t* lengths,
    ...
    for (int i = 0; i < out_count; i++) {
        ...
        if (desc_used_) desc_used_[idx] = true;
        idx = descs_[idx].next;
    }
...
bool Virtqueue::get_used(uint16_t* id, uint32_t* len) {
    ...
    *len = used_->ring[used_slot].len;
    ...
    uint16_t idx = *id;
    while (true) {
        if (desc_used_) desc_used_[idx] = false;
        num_free_++;
