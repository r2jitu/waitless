...
// Forward-declare serial::printf to avoid header dep cycle (see pci.cc comment).
...
static bool parse_capabilities(Device* dev) {
    ...
    while (cap_ptr != 0) {
        ...
        if (cap_vndr == PCI_CAP_ID_VNDR) {
            ...
            //   +8: offset(32)
            //   +12: length(32)
            uint32_t bar_word = pci::read_config(pci->bus, pci->slot, pci->func, cap_ptr + 4);
            uint8_t bar_idx = bar_word & 0xFF;
            uint32_t offset = pci::read_config(pci->bus, pci->slot, pci->func, cap_ptr + 8);
            // uint32_t length = pci::read_config(pci->bus, pci->slot, pci->func, cap_ptr + 12);
            ...
            case VIRTIO_PCI_CAP_NOTIFY_CFG:
                ...
                dev->notify_off_multiplier = pci::read_config(
                    pci->bus, pci->slot, pci->func, cap_ptr + 16);
                found_notify = true;
                break;
...
Device* find(uint16_t virtio_device_type) {
    ...
    if (device_count_ >= MAX_DEVICES) return nullptr;
...
void enable_queue(Device* dev) {
    w16(dev->common_cfg, CC_QUEUE_ENABLE, 1);
}
