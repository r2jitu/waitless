...
static constexpr uint32_t MMIO_DRIVER_FEATURES_SEL =
    0x024; // v2: select driver feature word
static constexpr uint32_t MMIO_QUEUE_READY =
    0x044; // v2: write 1 to activate queue
static constexpr uint32_t MMIO_QUEUE_DESC_LOW =
    0x080; // v2: descriptor table lo 32b
static constexpr uint32_t MMIO_QUEUE_DESC_HIGH =
    0x084; // v2: descriptor table hi 32b
static constexpr uint32_t MMIO_QUEUE_DRIVER_LOW =
    0x090; // v2: available ring lo 32b
static constexpr uint32_t MMIO_QUEUE_DRIVER_HIGH =
    0x094; // v2: available ring hi 32b
static constexpr uint32_t MMIO_QUEUE_DEVICE_LOW = 0x0a0; // v2: used ring lo 32b
static constexpr uint32_t MMIO_QUEUE_DEVICE_HIGH =
    0x0a4; // v2: used ring hi 32b
...
static uint64_t base_ =
    0; // non-zero = init complete (MMIO addr or PCI sentinel)
...
static inline volatile uint16_t &tx_avail_ring(int i) {
  return *reinterpret_cast<volatile uint16_t *>(tx_mem_ + AVAIL_OFFSET + 4 +
                                                i * 2);
}
...
static inline volatile uint16_t &rx_avail_ring(int i) {
  return *reinterpret_cast<volatile uint16_t *>(rx_mem_ + AVAIL_OFFSET + 4 +
                                                i * 2);
}
...
static inline volatile uint32_t &rx_used_ring_id(int i) {
  return *reinterpret_cast<volatile uint32_t *>(rx_mem_ + USED_OFFSET + 4 +
                                                i * 8);
}
...
static bool init_pci_queue(virtio_pci::Device *dev, uint32_t qidx,
                           uint8_t *mem) {
...
bool init_pci(virtio_pci::Device *dev) {
  ...
  // Activate device — must happen before RX kick (VZ ignores pre-DRIVER_OK
  // kicks)
  virtio_pci::set_status(dev, STATUS_ACKNOWLEDGE | STATUS_DRIVER |
...
bool init(uint64_t base_addr) {
  auto r32 = [base_addr](uint32_t off) {
    return arch::mmio_read32(base_addr + off);
  };
  auto w32 = [base_addr](uint32_t off, uint32_t v) {
    arch::mmio_write32(base_addr + off, v);
  };

  if (r32(MMIO_MAGIC_VALUE) != MMIO_MAGIC)
    return false;
  uint32_t ver = r32(MMIO_VERSION);
  if (ver != 1 && ver != 2)
    return false; // unsupported transport version
  if (r32(MMIO_DEVICE_ID) != 3)
    return false; // must be DeviceID=3 (console)
  ...
  if (is_v2)
    final_status |= STATUS_FEATURES_OK;
...
void putc(char c) {
  ...
  while (tx_used_idx_reg() == tx_last_used_)
    arch::cpu_relax();
