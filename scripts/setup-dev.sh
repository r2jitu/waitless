...
if [ "$HOST_ARCH" = "arm64" ]; then
    ...
    echo "# Set the platform directly (not via --config=aarch64) so that passing" >> "$BAZELRC_LOCAL"
    echo "# --config=aarch64 on the command line doesn't expand the config twice." >> "$BAZELRC_LOCAL"
    echo "build --platforms=//bazel/platforms:macos_arm64_unikernel" >> "$BAZELRC_LOCAL"
    echo "  → macos_arm64 set as default (Apple Silicon detected)"
else
    echo "# Generated by setup-dev.sh — do not commit" > "$BAZELRC_LOCAL"
