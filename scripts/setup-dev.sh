...
if [ "$HOST_ARCH" = "arm64" ]; then
    ...
    echo "# Default: native macOS arm64 (fast iteration, no VM required)" >> "$BAZELRC_LOCAL"
    echo "build --platforms=//bazel/platforms:native_macos_arm64" >> "$BAZELRC_LOCAL"
    echo "# config=qemu: use aarch64 unikernel platform" >> "$BAZELRC_LOCAL"
    echo "build:qemu --platforms=//bazel/platforms:aarch64_unikernel" >> "$BAZELRC_LOCAL"
    echo "  → native_macos_arm64 set as default (Apple Silicon detected)"
else
    echo "# Generated by setup-dev.sh — do not commit" > "$BAZELRC_LOCAL"
    echo "# Default: x86_64 unikernel (set globally in .bazelrc)." >> "$BAZELRC_LOCAL"
    echo "# config=qemu: x86_64 unikernel platform" >> "$BAZELRC_LOCAL"
    echo "build:qemu --platforms=//bazel/platforms:x86_64_unikernel" >> "$BAZELRC_LOCAL"
