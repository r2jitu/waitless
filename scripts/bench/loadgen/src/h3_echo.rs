// HTTP/3 bidirectional (RX+TX) echo throughput workload.
//
// The QUIC analogue of a balanced request/response stress: each of
// `parallelism` worker tasks owns its own QUIC connection and, after
// the one-time handshake (skipped from the histogram), loops sending a
// POST to `/echo` with a `--body-bytes` request body, then reads the
// full echoed response body back to completion. The server's `/echo`
// endpoint reads the request body and writes it back verbatim, so this
// exercises BOTH directions per iteration: the QUIC RX path (decode +
// reassemble the uploaded STREAM frames) and the TX path (AEAD +
// packet-encode + UDP-emit the echoed body). The round-trip latency and
// completed-echoes-per-second isolate the bidirectional hot path that
// the buffered/streamed `/echo` handler drives.
//
// A round-trip is a FAILURE (not a slow success) if the send errors,
// the response read times out, OR the echoed length != the sent length
// — a mismatch means the server dropped/duplicated body bytes.
//
...
pub async fn run(
    ...
    let server_addr = match resolve(host, port) {
        Some(a) => a,
        None => {
            eprintln!("h3_echo: failed to resolve {host}:{port}");
    ...
    // The request body is the same `body_bytes` zeros every iteration —
    // `Bytes` is cheaply cloneable (ref-counted), so each `send_data`
    ...
    for worker_idx in 0..parallelism {
        ...
        let h = tokio::spawn(async move {
            ...
            // Round-trips that did NOT complete cleanly: send/recv error,
            // a timeout, or an echoed-length mismatch. A timed-out body
            ...
            let mut quinn_ep = match Endpoint::client(bind) {
                Ok(e) => e,
                Err(e) => {
                    eprintln!("h3_echo[{worker_idx}]: bind failed: {e}");
            ...
            .await
            {
                Ok(Ok(c)) => c,
                Ok(Err(e)) => {
                    eprintln!("h3_echo[{worker_idx}]: connect failed: {e}");
                    return (count_post, failures_post, hist);
                }
                Err(_) => {
                    eprintln!("h3_echo[{worker_idx}]: connect timed out");
                    return (count_post, failures_post, hist);
                }
            };
            ...
            let (mut driver, mut send_request) = match h3::client::new(h3_conn).await {
                Ok(p) => p,
                Err(e) => {
                    eprintln!("h3_echo[{worker_idx}]: h3 init failed: {e}");
                    return (count_post, failures_post, hist);
                }
            };
            ...
            while Instant::now() < deadline {
                ...
                let mut stream = match timeout(PER_OP_TIMEOUT, send_request.send_request(req)).await {
                    ...
                    Ok(Err(e)) => {
                        log_first(&mut logged_send_err, |once| {
                            eprintln!("h3_echo[{worker_idx}]: send_request err (logged once): {e}");
                            *once = true;
                        });
                        if post_warmup {
                            failures_post += 1;
                    ...
                    Err(_) => {
                        log_first(&mut logged_send_timeout, |once| {
                            eprintln!("h3_echo[{worker_idx}]: send_request timed out (logged once)");
                            *once = true;
                        });
                        if post_warmup {
                            failures_post += 1;
                ...
                // Send the request body (the bytes the server echoes),
                // then FIN the send side with `finish()`.
                match timeout(PER_OP_TIMEOUT, stream.send_data(body.clone())).await {
                    Ok(Ok(())) => {}
                    _ => {
                        log_first(&mut logged_data_err, |once| {
                            eprintln!("h3_echo[{worker_idx}]: send_data failed (logged once)");
                            *once = true;
                        });
                        if post_warmup {
                            failures_post += 1;
                ...
                match timeout(PER_OP_TIMEOUT, stream.recv_response()).await {
                    ...
                    Ok(Err(e)) => {
                        log_first(&mut logged_recv_err, |once| {
                            eprintln!("h3_echo[{worker_idx}]: recv_response err (logged once): {e}");
                            *once = true;
                        });
                        if post_warmup {
                            failures_post += 1;
                    ...
                    Err(_) => {
                        log_first(&mut logged_recv_timeout, |once| {
                            eprintln!(
                                "h3_echo[{worker_idx}]: recv_response timed out (logged once)"
                            );
                            *once = true;
                        });
                        if post_warmup {
                ...
                // Read the full echoed body, counting bytes. A clean
                // round-trip both reaches end-of-body AND echoes exactly
                // `body_bytes` back.
                ...
                if post_warmup {
                    if completed && echoed == body_bytes {
                        let elapsed_us = t0.elapsed().as_micros() as u64;
                        let _ = hist.record(elapsed_us.max(1));
                        count_post += 1;
                    } else {
                        if completed && echoed != body_bytes {
                            log_first(&mut logged_mismatch, |once| {
                                eprintln!(
                                    "h3_echo[{worker_idx}]: echoed length mismatch \
                                     (got {echoed}, sent {body_bytes}) (logged once)"
                                );
                                *once = true;
                            });
                        }
    ...
    // Surface incomplete / mismatched round-trips so a stalled or
    // corrupted echo reads as a failure, not a phantom slow "success".
    // The harness parses only the RPS / P50_US / P99_US lines; this is
    // diagnostic.
    eprintln!("h3_echo: completed={total} failed={total_failures}");
