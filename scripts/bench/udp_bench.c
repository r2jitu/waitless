...
static void run_sync(const char *host, int port, int duration, int collect_lat,
  ...
  while (1) {
    ...
    if (t1.tv_sec + t1.tv_nsec / 1e9 >= end)
      break;
  }
...
static void run_async_multi(const char *host, int port, int duration,
  ...
  while (1) {
    ...
    while (total_sent < target && burst < 64) {
      ssize_t r =
          sendto(fds[rr], sbuf, 5, 0, (struct sockaddr *)&dst, sizeof(dst));
      if (r > 0) {
        sent[rr]++;
        total_sent++;
    ...
    if (total_rate > 0 && total_sent >= target) {
      double next_us =
          start + ((double)(total_sent + 1) * 1e6 / (double)total_rate);
...
static void *async_thread_entry(void *arg) {
  ...
  run_async_multi(a->host, a->port, a->duration, a->n_senders, a->rate_pps,
                  a->out);
  return NULL;
}
...
static void run_concurrent(const char *host, int port, int duration,
                           int n_slots, long timeout_us, struct result *out) {
  struct sockaddr_in dst = {0};
  if (resolve_host(host, port, &dst) != 0) {
    fprintf(stderr, "resolve_host(%s) failed\n", host);
    return;
  ...
  while (1) {
    ...
    for (int i = 0; i < n_slots; i++) {
      if (outstanding[i])
        continue;
      ssize_t r =
          sendto(fds[i], sbuf, 5, 0, (struct sockaddr *)&dst, sizeof(dst));
      if (r > 0) {
        outstanding[i] = 1;
        send_time_us[i] = now;
        total_sent++;
...
static void *concurrent_thread_entry(void *arg) {
  ...
  run_concurrent(a->host, a->port, a->duration, a->n_slots, a->timeout_us,
                 a->out);
  return NULL;
}
...
int main(int argc, char **argv) {
  ...
  for (int i = 4; i < argc; i++) {
    if (strcmp(argv[i], "--async") == 0)
      ...
      rate_pps = atol(argv[i] + 7);
    else if (strncmp(argv[i], "--client-cpus=", 14) == 0)
      client_cpus = atoi(argv[i] + 14);
    else if (strncmp(argv[i], "--timeout=", 10) == 0)
      timeout_ms = atol(argv[i] + 10);
  }
  ...
  if (client_cpus < 1)
    client_cpus = 1;
  if (!concurrent_mode && client_cpus > senders)
    client_cpus = senders;
  if (client_cpus > 32)
    client_cpus = 32;
  ...
  struct result *results = mmap(NULL, shm_size, PROT_READ | PROT_WRITE,
  ...
  if (concurrent_mode) {
    ...
    for (int t = 0; t < client_cpus; t++) {
      ...
      if (pthread_create(&tids[n_threads], NULL, concurrent_thread_entry,
                         &targs[n_threads]) != 0) {
    ...
    for (int t = 0; t < client_cpus; t++) {
      ...
      if (pthread_create(&tids[n_threads], NULL, async_thread_entry,
                         &targs[n_threads]) != 0) {
        perror("pthread_create");
  ...
  if (concurrent_mode) {
    ...
    double loss_pct =
        total_sent > 0 ? 100.0 * (double)total_lost / (double)total_sent : 0.0;
    ...
    if (async_mode) {
      ...
      fprintf(stderr, "  Recv rate:  %.0f pkt/s (%.0f%% reply)\n", pps,
              100.0 * total_recv / (total_sent ? total_sent : 1));
    } else {
      printf("RESULT %.0f %ld %ld %ld\n", pps, p50, p99, p999);
