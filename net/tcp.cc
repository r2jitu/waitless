...
static void send_segment(Connection *conn, uint8_t flags, const void *data,
                         size_t len) {
  ...
  tcp->checksum =
      tcp_checksum(config.ip, conn->remote_ip, ipv4::PROTO_TCP, buf, total);
...
static void send_rst(Ipv4Addr remote_ip, uint16_t local_port,
                     uint16_t remote_port, uint32_t seq_num, uint32_t ack_num) {
  ...
  tcp->checksum =
      tcp_checksum(config.ip, remote_ip, ipv4::PROTO_TCP, buf, total);
...
Connection *accept(Connection *listener) {
  ...
  for (int i = 0; i < MAX_CONNECTIONS; i++) {
    ...
    if (c->state == State::ESTABLISHED &&
        c->listener_port == listener->local_port && !c->accepted) {
      c->accepted = true;
      return c;
    }
  }
...
void poll() { virtio_net::poll(ethernet::receive); }
