...
static uint8_t *build_dhcp_frame(const DhcpPacket *dhcp_pkt, size_t dhcp_len,
...
static uint8_t parse_options(const DhcpPacket *pkt, Ipv4Addr *subnet,
                             Ipv4Addr *router, Ipv4Addr *dns,
                             Ipv4Addr *server_id) {
  uint8_t msg_type = 0;
  subnet->addr = 0;
  router->addr = 0;
  ...
  while (i < sizeof(pkt->options)) {
    ...
    if (i >= sizeof(pkt->options))
      break;
    uint8_t opt_len = opts[i++];
    if (i + opt_len > sizeof(pkt->options))
      break;
...
static void dhcp_receive(const uint8_t *data, uint32_t len) {
  ...
  if (len <
      sizeof(EthernetHeader) + sizeof(Ipv4Header) + sizeof(UdpHeader) + 240) {
    return;
  }
  ...
  if (ethertype != ETHERTYPE_IPV4) {
    ...
    if (ethertype == ETHERTYPE_ARP) {
      arp::receive(data + sizeof(EthernetHeader), len - sizeof(EthernetHeader));
    }
    return;
  }
  ...
  const DhcpPacket *pkt =
      (const DhcpPacket *)((const uint8_t *)udp + sizeof(UdpHeader));

  // Verify this is a reply and matches our transaction ID
  if (pkt->op != 2)
    return; // Not a reply
  if (pkt->xid != current_xid)
  ...
  if (msg_type == DHCP_OFFER && !got_offer) {
    ...
    serial::printf("dhcp: offer received: %d.%d.%d.%d\n",
                   offered_ip.addr & 0xFF, (offered_ip.addr >> 8) & 0xFF,
...
bool discover() {
  ...
  serial::printf("dhcp: configured IP %d.%d.%d.%d\n", config.ip.addr & 0xFF,
                 (config.ip.addr >> 8) & 0xFF, (config.ip.addr >> 16) & 0xFF,
  ...
  serial::printf("dhcp: DNS %d.%d.%d.%d\n", config.dns.addr & 0xFF,
                 (config.dns.addr >> 8) & 0xFF, (config.dns.addr >> 16) & 0xFF,
