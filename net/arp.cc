...
void receive(const uint8_t *data, size_t len) {
  ...
  if (op == OP_REQUEST) {
    ...
    if (pkt->target_ip == config.ip && config.ip != IP_ANY) {
      ...
      ethernet::send(pkt->sender_mac, ethernet::ETHERTYPE_ARP, &reply,
                     sizeof(reply));
    }
  }
...
MacAddr resolve(Ipv4Addr ip) {
  ...
  // Retry up to 3 times with ~2 second timeout each (approximated by poll
  // iterations)
  ...
  serial::printf("arp: failed to resolve %d.%d.%d.%d, using broadcast\n",
                 ip.addr & 0xFF, (ip.addr >> 8) & 0xFF, (ip.addr >> 16) & 0xFF,
                 (ip.addr >> 24) & 0xFF);
  return MAC_BROADCAST;
}
