// crates/proto/field-huffman — RFC 7541 Appendix B static Huffman code.
//
// The HTTP field-compression Huffman code is identical for HPACK
// (HTTP/2, RFC 7541 §5.2) and QPACK (HTTP/3, RFC 9204 §4.1.4), so
// both `proto/http2` and `proto/http3` share this one decoder. This
// module implements the decode side (we never set H=1 on encode —
// emitting raw octets is always valid and saves complexity).
//
...
/// from `http3::listen` / `http2::serve_conn`'s one-time init so
/// the ~4 KiB tree allocation lands in the heap *before*
/// `waitless::set_ready` snapshots HEAP_BASELINE — without this, the
/// first H2/H3 request charges the alloc to the leak-check delta as a
/// one-time first-use cost and trips `HEAP_LEAK_CHECK LEAK` for tests
/// that run a single conn.
///
