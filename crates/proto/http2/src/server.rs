...
impl ResponseSink for H2Sink {
    ...
    fn write_chunk(&mut self, buf: IOBuf) -> Pin<Box<dyn Future<Output = Result<(), ()>> + '_>> {
        ...
        // `len` is read out before `buf` moves into the future; `push_back`
        // moves `buf` only on the returning path, so the retry back-edge never
        // sees a moved-from value (NLL).
