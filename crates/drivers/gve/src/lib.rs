...
use rx_coalesce::RxCoalescer;
use sync::Spinlock;

// ---- DQO multi-buf RX coalescer storage (item I) ---------------------------
//
// Per-qp [`RxCoalescer`] (the multi-buffer accumulator state machine,
// `rx_coalesce` crate) that `dqo::poll_qp_inner` feeds non-EOP RX
// completions into and emits on EOP. Kept in a static array indexed by
// qp rather than in `RxQueue` itself so `RxQueue` stays auto-`Sync`
// (atomic-only fields); the `unsafe impl Sync` lives on this newtype,
// scoped to exactly the one field that needs lock-free interior
// mutability. Mirrors virtio-net's `NetCell` pattern.
//
...
// window is single-threaded. The poll sites probe `is_pending()` via a
// shared read and form a `&mut` only when a chain is actually pending
// (steady per-core polling only), so the boot→steady handoff window
// does only concurrent shared reads. Drop callbacks (`dqo::dqo_repost`)
// run on the consumer core but never touch this slot.
pub(crate) struct PendingChainCell(pub(crate) UnsafeCell<RxCoalescer>);
...
pub(crate) static PENDING_CHAINS: [PendingChainCell; MAX_QUEUE_PAIRS] =
    [const { PendingChainCell(UnsafeCell::new(RxCoalescer::new())) }; MAX_QUEUE_PAIRS];
