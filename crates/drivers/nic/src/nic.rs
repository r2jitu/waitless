...
// registration, and installs the first success into the active-ops
// slot. Before that — and on native, where no section exists —
// `active_ops()` resolves to the null-object driver (all no-ops), so
// every dispatcher below is a single load + vtable call with no
// null branch.
...
pub fn init() -> bool {
    for reg in linked_ethernet_drivers() {
        if reg.nic.nic.probe() {
            set_active_ops(reg.nic);
            return true;
        }
    }
    false
...
pub fn submit_tx(handle: nic_api::TxBufHandle, frame_len: usize, csum: nic_api::CsumOffload) {
    active_ops().submit_tx(handle, frame_len, csum)
}
...
pub fn submit_tx_tso(
    ...
    active_ops().submit_tx_tso(handle, frame_len, hdr_len, csum_start, gso_size)
}
...
pub fn acquire_tx_udp_gso_buf() -> Option<nic_api::TxUdpGsoBufHandle> {
    active_ops().acquire_tx_udp_gso_buf()
}
...
/// segment. See [`nic_api::Nic::submit_tx_udp_gso`] for
/// field semantics.
pub fn submit_tx_udp_gso(
    ...
    active_ops().submit_tx_udp_gso(handle, frame_len, hdr_len, csum_start, gso_size)
}
...
/// (polling drivers only — see [`nic_api::Nic::arm_rx_idle`]). Returns
