...
/// # Safety
///
/// Implementers MUST be `#[repr(C, packed)]` (alignment 1) AND any bit
/// pattern of `size_of::<Self>()` bytes must be a valid `Self`.
/// Concretely: integer fields, byte arrays, and other plain `Copy` POD
/// types are fine; `bool`, `enum`, references, and `NonZero*` are NOT.
