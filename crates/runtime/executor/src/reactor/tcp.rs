...
// The guard type itself lives in `iobuf` (it is a pure wrapper over
// `iobuf::IOBuf` with a borrow phantom — no reactor state), so the
// `HttpStream` trait and the TLS stream can name it without depending
// on this crate. Re-exported here so `waitless::runtime::RecvChunkGuard`
// and every existing path keep working.
pub use iobuf::RecvChunkGuard;
