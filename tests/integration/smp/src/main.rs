...
fn init() {
    ...
    {
        let before = kernel_bare::aarch64::smp::ipi_count();
        kernel_bare::aarch64::smp::send_sgi_to(1);
        ...
        let deadline = kernel_bare::time::now_cycles()
            .wrapping_add(1_000_000 * kernel_bare::time::cycles_per_us());
        while kernel_bare::aarch64::smp::ipi_count() == before
            && kernel_bare::time::now_cycles() < deadline
        {
            unsafe {
                core::arch::asm!("yield");
            }
