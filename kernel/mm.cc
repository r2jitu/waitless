...
void init(uint64_t boot_info_addr) {
    ...
    for (uint64_t i = 0; i < frame_bitmap_size; i++) frame_bitmap[i] = 0xFF;
    ...
    for (uint64_t addr = free_start; addr < max_physical_addr; addr += PAGE_SIZE) {
    ...
    if (mbi_addr != 0 && *magic_ptr == HVM_START_MAGIC_VALUE) {
        ...
        const HvmMemmapEntry* entries =
            reinterpret_cast<const HvmMemmapEntry*>(
                (uint64_t)info->memmap_paddr);
        ...
        for (uint32_t i = 0; i < info->memmap_entries; i++) {
            if (entries[i].type != HVM_MEMMAP_TYPE_RAM) continue;
        ...
        uint32_t mb2_total_size = mbi_addr ? *reinterpret_cast<const uint32_t*>(mbi_addr) : 0;
        if (mb2_total_size >= 8 && mb2_total_size < 65536) {
            uint64_t tag_addr = mbi_addr + 8;
            while (true) {
                const MultibootTag* tag = reinterpret_cast<const MultibootTag*>(tag_addr);
            ...
            if (mmap_tag) multiboot2_valid = true;
        }

        if (multiboot2_valid) {
            uint64_t mmap_entries_start = reinterpret_cast<uint64_t>(&mmap_tag->entries[0]);
            uint64_t mmap_entries_end   = reinterpret_cast<uint64_t>(mmap_tag) + mmap_tag->size;
            uint32_t entry_size = mmap_tag->entry_size;

            for (uint64_t ea = mmap_entries_start; ea < mmap_entries_end; ea += entry_size) {
                const MultibootMmapEntry* e = reinterpret_cast<const MultibootMmapEntry*>(ea);
            ...
            uint32_t mbi_total_size_val = *reinterpret_cast<const uint32_t*>(mbi_addr);
    ...
    while (current) {
        if (current->free && current->size >= size) {
            ...
            return reinterpret_cast<void*>(
                reinterpret_cast<uint8_t*>(current) + sizeof(BlockHeader));
        }
